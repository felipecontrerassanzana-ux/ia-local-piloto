...
.SINOPSIS
  ...
  Ver docs/06-operacion/02-acceso-remoto.md para la decisión y docs/06-operacion/01-mantenimiento.md para la continuidad.
...
.PARAMETER Token
  El token del túnel, obtenido así (paso manual, no se puede automatizar — es específico de tu cuenta):
    ...
    4. En "Public Hostname", apuntar el subdominio elegido (ver docs/01-arquitectura/04-almacenamiento.md) al servicio
       local http://localhost:8080 (Open WebUI nativo, ver docs/01-arquitectura/03-docker-y-recursos.md — no Ollama directo).
#>
...
Write-Host "queda anotado en el checklist mensual de docs/06-operacion/01-mantenimiento.md, revisar manualmente." -ForegroundColor Yellow
