...
| `bitacora-horas.bat` | Genera `../logs/bitacora-horas.html` — cuánto tiempo activo real (y tokens) se trabajó en cada proyecto, día por día, a partir de los logs de sesión de Claude Code de esta cuenta de Windows. Servido también en `http://localhost:8090/bitacora` una vez generado (ver `../docs/06-operacion/03-monitor-estado.md`). Necesita `bitacora-proyectos.json` (copiar de `.example.json` y completar con tus propios proyectos — queda fuera de git). |
