...
Primera pasada del material de estudio pedido (por herramienta/modelo, paso a paso). Se crearon `docs/referencia/01-ollama.md` (CLI completo, API, variables de entorno de rendimiento no cubiertas antes en el proyecto: `keep_alive`, Flash Attention, cuantización de KV cache) y `docs/referencia/02-qwen-2.5-coder-7b.md`.
...
Segunda pasada del material de estudio (paso a paso, según lo acordado). Se crearon `docs/referencia/03-qwen-code.md` y `docs/referencia/04-goose.md` con el set real de comandos de cada herramienta — más grande de lo que se había cubierto hasta ahora en `01-herramientas-trabajo.md`/`02-qwen-code-a-fondo.md`.
...
- Se crearon `docs/referencia/05-open-webui.md` (con el hallazgo completo) y `docs/referencia/06-qdrant.md` (conceptos y API básica, incluyendo la dimensión correcta de vector para BGE-M3: 1024).
...
- Se actualizaron `scripts/README.md`, `docs/instalacion/01-plan-instalacion.md`, `docs/instalacion/02-aprendizaje-scripts.md`, `AGENTS.md` y `README.md` para reflejar el nuevo instalador como el camino recomendado, dejando los 17 scripts individuales disponibles para correr un paso puntual de nuevo.
...
- `scripts/pasos/monitor-estado-servidor.ps1` — servidor HTTP nativo (`System.Net.HttpListener`, sin dependencias nuevas) con `GET /estado` (JSON) y `GET /` (dashboard HTML, mismo tema oscuro del instalador). Chequea lo mismo que `verificar-instalacion.ps1` (Ollama, Qdrant, Open WebUI, cloudflared, Tailscale, ComfyUI, backup) más GPU/VRAM, discos y CPU/RAM — duplicación deliberada, no un descuido (ver `docs/operacion/03-monitor-estado.md`).
...
- Las carpetas mismas (`docs/arquitectura/`, `docs/modelo/`, etc.) **no** se numeraron — es una decisión aparte, más riesgosa (rompe más referencias: scripts, tabla del README raíz, links entre subcarpetas), que queda pendiente de que Felipe confirme si también la quiere.
...
- Se aprovechó la pasada para corregir dos bugs de rutas que ya estaban rotos antes de esto (no causados por el renombrado): una profundidad de ruta incorrecta hacia `ia-local` en `docs/modelo/02-modelo-elegido.md`, y una mención bare de `modelos.md` sin la ruta completa en el mismo archivo.
