...
**Estado al cierre de esta entrada:** estructura de documentos creada (README, 03-hardware-real.md, 02-modelo-elegido.md, 01-plan-instalacion.md, 01-plan-pruebas.md, decisiones.md) — ningún paso de instalación real ejecutado todavía en el equipo.
...
- **Motor — decisión cerrada:** Ollama para partir (mejor soporte en Continue.dev/Aider, más simple), LocalAI queda como escalón si en el uso real aparece necesidad concreta de otras funciones (voz/imagen) — no antes. Ver `herramientas/04-motor-alternativas.md`.
...
Se actualizó `operacion/02-acceso-remoto.md` con la respuesta y los próximos pasos concretos (configurar Cloudflare Access con la lista de correos autorizados, antes de dejar el túnel activo).

## 2026-08-26 (mismo día) — Esquema estructural completo + revisión proactiva

El usuario pidió juntar todo lo decidido hasta ahora en un esquema estructural del montaje del proyecto, y pidió explícitamente que fuera propositivo: señalar qué más podría hacer falta, no solo responder lo preguntado. Se creó `arquitectura/01-arquitectura-piloto.md` con el diagrama completo del stack (Cloudflare Access → Cloudflare Tunnel → Open WebUI → Ollama → RAG, y por separado Goose/Continue.dev en uso local) y una tabla que conecta cada capa con el documento que la fundamenta.
...
- **Actualizaciones ("el P5"):** Felipe pidió formularlo en serio, no dejarlo vago — se creó `operacion/01-mantenimiento.md` con checklist mensual concreto (Ollama, modelo, Goose, Continue.dev, cloudflared, Open WebUI, verificación de backup y de servicios de inicio).
- **Costos:** Felipe pidió confirmar que toda la estructura sea gratis. Verificado directamente en la página oficial de precios de Cloudflare: **Cloudflare Access es gratis hasta 50 usuarios** y **Cloudflare Tunnel no tiene costo aparte** (incluido en cualquier cuenta). Se armó una tabla completa en `operacion/01-mantenimiento.md` confirmando que cada pieza del stack (Ollama, Qwen, Goose, Continue.dev, Aider, Open WebUI, BGE-M3, Qdrant, Mem0) es gratuita/self-hosted — no hay ningún costo recurrente nuevo en todo el diseño.

Se creó `operacion/01-mantenimiento.md` y se actualizaron `modelo/03-hardware-real.md`, `operacion/02-acceso-remoto.md` y `arquitectura/01-arquitectura-piloto.md` con todo lo anterior. Del listado proactivo original, el único punto que sigue realmente abierto es el **sistema operativo** (Windows vs. Linux).

## 2026-08-26 (mismo día) — SO confirmado, corrección de contenido personal, scripts reales y GitHub

Felipe confirmó **Windows 11 Pro 25H2**, con dos discos (NVMe ~500GB + HDD 1TB, reparto sin definir) — cierra el último bloqueante que quedaba abierto. Se creó `arquitectura/04-almacenamiento.md` con el criterio de reparto (modelo/backups en HDD, SO/proyectos activos/RAG en NVMe) y `OLLAMA_MODELS` como mecanismo concreto.

**Corrección de contenido:** Felipe pidió explícitamente que la documentación oficial no mencione que el equipo es de un familiar — ese dato es solo referencial para la conversación (surgió en un momento anterior), no necesario para el proyecto. Se corrigió `modelo/03-hardware-real.md` y esta bitácora para hablar solo de "uso compartido" sin especificar de quién. **Aprendizaje para aplicar hacia adelante:** no todo lo que se conversa en el contexto de una sesión debe quedar escrito en la documentación de un proyecto — detalles personales o familiares mencionados de pasada se mantienen fuera salvo pedido explícito, igual que ya se aplica con el contenido específico de empresa entre `ia-local` y los proyectos de Tecnoingeniería.
...
- Se creó `pruebas/02-pruebas-rendimiento.md`: explica qué mide cada prueba, cómo interpretar los resultados (ej. degradación en carga sostenida como señal de throttling térmico, verificado cruzando con lecturas de `nvidia-smi`), y cómo esta prueba cierra específicamente el pendiente que había quedado abierto en `modelo/02-modelo-elegido.md` sobre si el límite de 32K de contexto de Ollama es real o solo un default de fábrica.
- Se actualizó `instalacion/01-plan-instalacion.md` con un Paso 6 (prueba de estrés) y una nota inicial apuntando a `instalacion/02-aprendizaje-scripts.md` antes de empezar a instalar.
...
Se creó `scripts/12-instalar-herramientas-dev.ps1` + `.bat` (pasó por `_verificar-sintaxis.ps1` sin hallazgos nuevos) y se actualizaron `herramientas/01-herramientas-trabajo.md` (nueva sección "Conectores a GitHub"), `instalacion/01-plan-instalacion.md` (nuevo Paso 1.4) y `scripts/README.md`.
...
Detalle completo, incluyendo la explicación del porqué de la diferencia entre intérpretes de PowerShell, en `instalacion/02-aprendizaje-scripts.md`.
...
Se actualizó `herramientas/01-herramientas-trabajo.md` con una sección nueva y se corrigió la tabla comparativa Qwen Code vs. Goose para incluir esta diferencia real.
...
- Se creó `herramientas/03-como-funcionan-los-agentes.md`: ciclo de vida de una sesión (contexto vacío → carga de archivos persistentes → tarea → actualización de memoria), tabla de qué archivo lee cada herramienta (`AGENTS.md`, `QWEN.md`, `.goosehints`, `.continue/rules`, memoria automática de Qwen Code), y explicación de MCP/SubAgents/Skills/extensiones.
...
- **Aclaración explícita:** usar Tailscale para llegar directo al puerto de Ollama no contradice la decisión ya cerrada de "no exponer el puerto crudo de Ollama" (`operacion/02-acceso-remoto.md`) — esa decisión era sobre exposición pública vía Cloudflare, y Tailscale es una red privada cerrada, no exposición pública.
...
- Se creó `herramientas/02-qwen-code-a-fondo.md` con el detalle completo (tabla de funciones de Qwen Code cruzadas contra los números reales de `modelo/02-modelo-elegido.md`, y los tres escenarios de conexión). Se actualizaron `arquitectura/01-arquitectura-piloto.md`, `operacion/02-acceso-remoto.md`, `operacion/01-mantenimiento.md` (Tailscale y Qwen Code agregados a la tabla de costos) y `README.md` para reflejarlo.
...
- Se corrigieron dos referencias obsoletas encontradas de paso: `scripts/README.md` y `instalacion/01-plan-instalacion.md` todavía mencionaban Open WebUI en el puerto 3000 y como "contenedor Docker" — arrastre de antes de la migración a nativo (puerto 8080, Tarea Programada) que no se había propagado a esos dos archivos.
- Se agregó el Paso 3.5 en `instalacion/01-plan-instalacion.md` para la instalación de Tailscale, dejado explícitamente como opcional y distinto del acceso por navegador del Paso 3.
...
- **Re-chequeado si hay un Qwen3-Coder chico (verificado en `github.com/QwenLM/Qwen3-Coder`):** no existe ninguna variante bajo los 30B en esa línea (solo 480B-A35B, 30B-A3B y Qwen3-Coder-Next, este sobre una base de 80B). **La elección de Qwen 2.5 Coder 7B se mantiene** — no por falta de revisión, sino porque no hay una opción más chica disponible todavía. Se documentó en `modelo/02-modelo-elegido.md` como una confirmación cerrada, no como una duda abierta.
...
- Se actualizaron `modelo/02-modelo-elegido.md`, `herramientas/02-qwen-code-a-fondo.md`, `instalacion/02-aprendizaje-scripts.md` y `scripts/13-instalar-qwen-code.ps1` con estos hallazgos.
...
- Documentado en `herramientas/03-como-funcionan-los-agentes.md`, nueva sección "Continuidad de sesión: qué pasa si se corta la luz o se apaga el equipo".
...
Documentado en `herramientas/03-como-funcionan-los-agentes.md` — el objetivo es dejar registrado que se investigó y por qué no hace falta acción, no solo omitirlo en silencio.
...
- **Corrección relacionada, encontrada al planear esto:** el criterio de `arquitectura/04-almacenamiento.md` decía que los modelos van al HDD porque "se leen una sola vez al cargar" — esa asunción ya no aplica con el diseño de intercambio de modelos bajo demanda (se lee el disco varias veces por sesión, no una vez). Se corrigió el criterio y el default de `02-configurar-ollama.ps1` (`-LetraNVMe` en vez de `-LetraHDD`) para que los modelos vayan al NVMe.
- Se creó `arquitectura/02-capa-diseno.md` con el detalle completo (incluyendo el loop generar→revisar→corregir, documentado como procedimiento — formalizarlo como Skill queda pendiente de verificar el formato exacto que usan Qwen Code/Goose).
...
- Se dejó reservado `docs/07-referencia/` (mencionado en el README principal) para el material de estudio por herramienta/modelo, a construirse en la siguiente pasada.

## 2026-08-27 (mismo día) — Material de referencia: Ollama y Qwen 2.5 Coder 7B, y un hallazgo real sobre el límite de contexto

Primera pasada del material de estudio pedido (por herramienta/modelo, paso a paso). Se crearon `docs/07-referencia/01-ollama.md` (CLI completo, API, variables de entorno de rendimiento no cubiertas antes en el proyecto: `keep_alive`, Flash Attention, cuantización de KV cache) y `docs/07-referencia/02-qwen-2.5-coder-7b.md`.

- **Hallazgo real, no solo de referencia:** investigando el `config.json` oficial del modelo (`huggingface.co/Qwen/Qwen2.5-Coder-7B-Instruct`), se confirmó que `max_position_embeddings` es `32768` — **32K es el contexto real de entrenamiento del modelo, no un default arbitrario de Ollama**. Los 131K/128K citados en `01-fundamentacion-modelo.md`/`02-modelo-elegido.md` como "contexto completo" requieren activar YaRN explícitamente (confirmado textual en la ficha oficial), algo que Ollama todavía no expone de forma completa (`github.com/ollama/ollama/issues/11871`). Extender el contexto sin YaRN no da error, pero es esperable que degrade la calidad de las respuestas que usan ese contexto extra — no solo un tema de que "no se caiga".
- Se actualizó `modelo/02-modelo-elegido.md` (el pendiente sobre 32K vs 100K, marcado ahora como investigado a fondo) y `pruebas/02-pruebas-rendimiento.md` (la prueba de rampa de contexto debe evaluar calidad además de si la petición se procesa, no alcanza con "no tira error").

## 2026-08-27 (mismo día) — Material de referencia: Qwen Code y Goose, y un hallazgo que simplifica el loop de la capa de diseño

Segunda pasada del material de estudio (paso a paso, según lo acordado). Se crearon `docs/07-referencia/03-qwen-code.md` y `docs/07-referencia/04-goose.md` con el set real de comandos de cada herramienta — más grande de lo que se había cubierto hasta ahora en `01-herramientas-trabajo.md`/`02-qwen-code-a-fondo.md`.

- **Hallazgo real:** Qwen Code tiene `/model --vision <modelo>` — un mecanismo nativo para configurar un modelo puente de visión (exactamente el rol de `qwen3-vl:4b`) cuando el modelo principal es de solo texto. Esto podría simplificar el loop de `02-capa-diseno.md` (pasos 2-3) en vez de armarlo como procedimiento manual. También existe `/model --image` para generación de imágenes incorporada — pendiente confirmar si ComfyUI puede exponerse compatible (su API es por workflow JSON, no un endpoint simple).
- **Resuelto el pendiente de "formalizar el loop como Skill":** no hacía falta una Skill — Qwen Code tiene comandos personalizados (`.qwen/commands/*.md`, versionables en el repo) y Goose tiene el concepto equivalente (Recipes). Se documentó el comando personalizado completo para Qwen Code en `referencia/03-qwen-code.md`; el Recipe de Goose queda pendiente de armar (formato exacto no verificado en detalle todavía).
- Se actualizó `arquitectura/02-capa-diseno.md` con ambos hallazgos y se cerraron/precisaron los pendientes de esa página.
...
- Se crearon `docs/07-referencia/05-open-webui.md` (con el hallazgo completo) y `docs/07-referencia/06-qdrant.md` (conceptos y API básica, incluyendo la dimensión correcta de vector para BGE-M3: 1024).
...
- Se actualizaron `scripts/README.md`, `docs/04-instalacion/01-plan-instalacion.md`, `docs/04-instalacion/02-aprendizaje-scripts.md`, `AGENTS.md` y `README.md` para reflejar el nuevo instalador como el camino recomendado, dejando los 17 scripts individuales disponibles para correr un paso puntual de nuevo.
...
- `scripts/pasos/monitor-estado-servidor.ps1` — servidor HTTP nativo (`System.Net.HttpListener`, sin dependencias nuevas) con `GET /estado` (JSON) y `GET /` (dashboard HTML, mismo tema oscuro del instalador). Chequea lo mismo que `verificar-instalacion.ps1` (Ollama, Qdrant, Open WebUI, cloudflared, Tailscale, ComfyUI, backup) más GPU/VRAM, discos y CPU/RAM — duplicación deliberada, no un descuido (ver `docs/06-operacion/03-monitor-estado.md`).
...
- Alcanzable por los dos mecanismos de acceso remoto ya decididos: local (`localhost:8090`), Tailscale directo, o agregando una segunda "Public Hostname" al mismo túnel de Cloudflare (paso manual, opcional, documentado en `02-acceso-remoto.md`) — sin crear infraestructura nueva.
...
## 2026-08-27 (mismo día) — Prefijo numérico en cada archivo de `docs/*/` para que el orden de lectura se vea sin abrir el README

Felipe notó que el orden de lectura solo estaba escrito en prosa dentro del `README.md` de cada subcarpeta (`docs/01-arquitectura/README.md`, `docs/02-modelo/README.md`, etc.) — si alguien mira una carpeta directo (Explorador, GitHub) sin leer ese README primero, no hay forma de saber cuál archivo va primero. Mismo criterio que ya se usaba en `scripts/pasos/` (prefijos `00-`, `01-`, ...), aplicado ahora a los 23 archivos de las 7 subcarpetas de `docs/`:

- El número de cada archivo coincide exactamente con el orden en que ya estaba listado en el `README.md` de su propia carpeta — no se re-decidió el orden, solo se hizo visible sin depender de leer prosa.
- Las carpetas mismas (`docs/01-arquitectura/`, `docs/02-modelo/`, etc.) **no** se numeraron — es una decisión aparte, más riesgosa (rompe más referencias: scripts, tabla del README raíz, links entre subcarpetas), que queda pendiente de que Felipe confirme si también la quiere.
- Todas las referencias cruzadas corregidas y re-verificadas programáticamente: dentro de este repo, dentro de `scripts/*.ps1`/`.bat` (comentarios y mensajes que citan rutas de docs), y hacia `ia-local` (que también se numeró el mismo día) — 0 links rotos, 0 anclas rotas, confirmado con un verificador que recorre ambos repos a la vez.
- Se aprovechó la pasada para corregir dos bugs de rutas que ya estaban rotos antes de esto (no causados por el renombrado): una profundidad de ruta incorrecta hacia `ia-local` en `docs/02-modelo/02-modelo-elegido.md`, y una mención bare de `modelos.md` sin la ruta completa en el mismo archivo.

## 2026-08-27 (mismo día) — También las 7 carpetas de `docs/` quedaron numeradas

Felipe confirmó explícitamente: "todo tiene que tener un orden... todas las organizaciones de carpetas y documentos tienen que tener un orden estructural" — no basta con numerar los archivos adentro de cada carpeta, las carpetas mismas también deben mostrar su orden sin depender de leer un README. Renombradas según el orden ya declarado en la tabla del `README.md` raíz: `docs/arquitectura/` → `01-arquitectura/`, `modelo/` → `02-modelo/`, `herramientas/` → `03-herramientas/`, `instalacion/` → `04-instalacion/`, `pruebas/` → `05-pruebas/`, `operacion/` → `06-operacion/`, `referencia/` → `07-referencia/`.

Esta vez sí se tocaron más referencias que la pasada anterior (renombrar una carpeta cambia la ruta de todo lo que hay adentro, no solo un archivo): la tabla del `README.md` raíz, cada `.ps1`/`.bat` que citaba una ruta de doc en un comentario o mensaje, los links entre subcarpetas hermanas (ej. `docs/03-herramientas/*.md` citando `../02-modelo/02-modelo-elegido.md`), y las referencias desde `ia-local` hacia estas carpetas. Re-verificado con el mismo script que recorre ambos repos a la vez (más `PROYECTOS.md`, ya relocalizado fuera de ambos): 0 links rotos, 0 anclas rotas.

**Alcance deliberadamente acotado:** no se numeraron las carpetas de nivel superior (`docs/`, `scripts/`, `plantillas/`, `logs/`) — son categorías paralelas y ya convencionales/reconocibles por nombre (cualquiera busca scripts en `scripts/`), no una secuencia de lectura como sí lo son las 7 subcarpetas de `docs/`. Si Felipe también las quiere numeradas, queda para una pasada aparte.

## 2026-08-27 (mismo día) — Cierre de `docs/07-referencia/`: Qwen3-VL, ComfyUI, Tailscale, Cloudflare

Felipe marcó que el doc de referencia estaba "al debe" — quedaban 4 de las 10 piezas del stack sin cubrir (`README.md` de la carpeta ya las listaba como pendientes: Qwen3-VL, ComfyUI, Tailscale, Cloudflare). Antes de escribir, Felipe cuestionó el enfoque: pidió confirmar que el contenido fuera específico de lo que realmente está montado en este piloto, no un manual genérico de cada herramienta. Se confirmó ese alcance explícitamente para cada uno (mismo criterio que ya aplicaban los 6 documentos anteriores, solo que no estaba dicho en voz alta): Qwen3-VL acotado a su rol de revisor visual (`/model --vision`, ver `../01-arquitectura/02-capa-diseno.md`), ComfyUI a la particularidad de su API de workflow-JSON y el checkpoint SD 1.5 ya elegido, Tailscale a conectar Qwen Code/Goose remotos (Escenario C1 de `../03-herramientas/02-qwen-code-a-fondo.md`), y Cloudflare a Tunnel remotely-managed + Access tal como está configurado (dos Public Hostnames, Service Tokens).

Se crearon `07-qwen3-vl.md`, `08-comfyui.md`, `09-tailscale.md`, `10-cloudflare.md`, verificados contra documentación oficial (`ollama.com`, `docs.comfy.org`, `tailscale.com/kb`, `developers.cloudflare.com/cloudflare-one`), 2026-08-27, y se actualizó el `README.md` de la carpeta quitando la nota "en construcción". Un hallazgo quedó marcado explícitamente como sin confirmar (no se inventó una respuesta): la documentación oficial de Cloudflare no dice si una ruta agregada en el dashboard de un túnel remotely-managed aplica sin reiniciar `cloudflared` — es consistente con el diseño (la config vive en Cloudflare, no en el equipo), pero queda pendiente de comprobar la primera vez que se agregue una ruta real.

`docs/07-referencia/` queda completo: 10 de 10 piezas del stack cubiertas.

## 2026-08-28 — Bitácora de horas por proyecto: de Artifact a script real, y la pregunta de multi-usuario

Felipe pidió cuantificar cuánto tiempo propio invierte trabajando en cada proyecto (indistinto de cuál), considerando los huecos donde no está — para saber cuánto de su tiempo real va a generar/pulir ideas. Se prototipó primero fuera de este repo, como Artifact de Claude (varias vueltas de diseño e iteración de layout), y una vez validado el concepto, a pedido explícito de Felipe se portó acá como script real — mismo argumento que ya cerró `03-monitor-estado.md` sobre por qué `/estado` no es un Artifact: no depender de claude.ai para que algo del propio equipo se actualice.

**Qué se construyó:**

- `scripts/bitacora-horas.ps1` — parsea `~/.claude/projects/**/*.jsonl` de la cuenta de Windows que lo corre. Método: ordenar cronológicamente los eventos de cada sesión, sumar el tiempo entre eventos consecutivos solo si el hueco es menor a un umbral configurable (default 20 min) — un hueco mayor se asume que la persona se fue. El proyecto de cada tramo se detecta mirando las rutas de archivo/comandos que las herramientas invocadas tocaron (no el texto de la conversación, para no confundir "mencionar un proyecto" con "trabajar en él"). Suma también tokens de entrada/salida por proyecto, mostrados aparte del tiempo — mismo criterio que planteó Felipe: el conteo de tokens varía mucho según la complejidad de la tarea, no es un buen proxy de esfuerzo, así que no se mezcla con la métrica de tiempo.
- `scripts/bitacora-proyectos.json` (fuera de git) + `.example.json` (versionado, genérico) — la lista de proyectos a reconocer se separó del script a propósito: los nombres de los otros repos de Felipe no pueden quedar hardcodeados en este repo, mismo criterio de independencia que ya se aplica entre `ia-local` y Tecnoingeniería (ver `PROYECTOS.md`).
- `scripts/bitacora-plantilla.html` — el HTML/CSS/JS del dashboard (diseño tipo "bitácora de papel/tinta": `Fraunces` + `JetBrains Mono`, sidebar de proyectos con descripción a la derecha, timeline de ventanas de tiempo real día por día, gráfico diario, distribución por hora). Reutilizado tal cual del prototipo del Artifact, ya verificado visualmente ahí — el trabajo nuevo fue la generación de datos en PowerShell, no el frontend.
- `monitor-estado-servidor.ps1` — nueva ruta `/bitacora`, sirve el HTML ya generado (no lo recalcula por request, parsear los `.jsonl` puede tardar y crece con el tiempo) más un link visible desde el dashboard principal.

**Verificado de verdad, no solo escrito:** corrido contra los logs reales de esta cuenta (17 archivos de sesión, 374 bloques fusionados) — generó el HTML correctamente, sin errores. Abierto con Playwright: 0 errores de consola, el layout responde bien en 1600px y 480px. Pasó por el mismo control de calidad que el resto del repo (parser de sintaxis, PSScriptAnalyzer, chequeo de BOM UTF-8 — faltaba, corregido).

**Límite documentado, no oculto:** `ConvertFrom-Json` en Windows PowerShell 5.1 (el intérprete real que usan los `.bat` de este proyecto) tiene un límite de ~2MB por línea — una línea de log más grande que eso (ej. el resultado de leer un archivo grande) se salta en silencio. Subestima el tiempo activo en ese tramo puntual, nunca lo sobreestima. No se resolvió con un parser propio (ej. `System.Text.Json`, sin ese límite pero no disponible sin más en .NET Framework/PowerShell 5.1) — sería sumar complejidad real para una herramienta de referencia personal, no crítica.

**La pregunta que Felipe planteó y quedó investigada, no resuelta del todo:** ¿cómo separar el uso de Felipe del de Vicente en el equipo compartido del piloto? Investigado contra documentación oficial (2026-08-28):

- **Qwen Code y Goose, corriendo contra el Ollama local de este piloto, no tienen ningún login ni cuenta asociada** — a propósito, para no depender de servicios externos. No hay correo ni ID que capturar ahí.
- **Open WebUI sí tiene multiusuario real** (login local, historial separado por `user_id`, confirmado en `docs.openwebui.com`) — cubre el uso por chat/RAG, pero no el de los agentes de código.
- **Conclusión aplicada:** para los agentes de código, la única separación real disponible es la **cuenta de Windows** — `bitacora-horas.ps1` ya lee de `$env:USERPROFILE`, así que corrido por cada persona con su propia cuenta ya da bitácoras separadas sin cambiar código. **Queda pendiente de decidir** (no de investigar) si Felipe y Vicente van a usar cuentas de Windows separadas en el equipo — eso sí resolvería esto del todo.

Todo commiteado y verificado con `_verificar-sintaxis.bat` antes de darse por terminado.

## 2026-08-28 (mismo día, más tarde) — Bug real: acentos corruptos al correr `bitacora-horas.bat` de verdad

Felipe corrió `bitacora-horas.bat` en el equipo real (no solo la verificación de sintaxis) y reportó que el resultado "no muestra nada, no abre". Investigado: el archivo **sí se generaba** (confirmado con `Get-ChildItem`, tamaño correcto) y el JS **sí funcionaba** (0 errores de consola, verificado con Playwright) — el problema real era que **todas las tildes/ñ/acentos salían corruptos** (`"BitÃ¡cora"` en vez de `"Bitácora"`, `"DÃ­as"` en vez de `"Días"`, etc.), suficiente para que la página se viera como texto ilegible y pareciera "rota" o "vacía" a simple vista.

**Causa raíz:** `Get-Content -Raw` en Windows PowerShell 5.1 (el intérprete real que usan los `.bat` de este proyecto, confirmado otra vez que **no** es lo mismo que pwsh 7 — mismo gotcha ya documentado en `04-instalacion/02-aprendizaje-scripts.md` para `_verificar-sintaxis.ps1`) no detecta UTF-8 sin BOM automáticamente — lo lee con el codepage del sistema (Windows-1252), corrompiendo cada carácter no-ASCII. Afectaba a las dos lecturas de archivo del script: `bitacora-proyectos.json` (contiene "Tecnoingeniería", "protección", etc.) y `bitacora-plantilla.html` (contiene "Bitácora", "días", tildes en toda la interfaz) — ninguno de los dos tiene BOM (los `.json`/`.html` no lo necesitan para nada más, solo para esta lectura puntual vía `Get-Content`).
...
## 2026-08-30 (mismo día) — El `README.md` raíz listaba los docs de `ia-local` fuera de orden

Felipe notó (comparando la lista de "Qué usa de `ia-local`" del `README.md` contra el índice completo de `ia-local/README.md`) que los 5 archivos referenciados no estaban en orden ascendente: `02, 03, 04, 01, 06` en vez de `01, 02, 03, 04, 06` — inconsistente con el criterio de numeración que el resto del proyecto sigue estrictamente. Corregido, y de paso agregado `05-metodologia-de-proyecto.md` a la lista (no estaba, aunque ese documento se escribió a partir de auditar este mismo piloto — tiene sentido que se referencie de vuelta). Se aprovechó la misma pasada para: (1) sacar la nota `(próximamente)` de `docs/07-referencia/` en la tabla de estructura, que ya está completo desde el 2026-08-27 y quedó sin actualizar; (2) mencionar `bitacora-horas.bat` en la lista de scripts, que faltaba.

**Barrido de verificación, no solo el caso puntual:** se revisaron los README raíz de los otros 3 repos y los 7 README de subcarpetas de `docs/` en este piloto buscando el mismo tipo de desorden — todos estaban correctos, este era el único caso. Reverificado con el script de links/anclas de siempre: 0 errores en los 73 `.md` de los cuatro repos.

## 2026-08-30 (mismo día, más tarde) — `03-monitor-estado.md` tenía contenido de otra funcionalidad mezclado

Felipe notó, leyendo la documentación, que `docs/06-operacion/03-monitor-estado.md` traía contenido sobre Vicente y sobre pedidos suyos que no parecían del monitor — como si fuera "memoria persistente tirada en un doc de documentación". Correcto: la sección `/bitacora` completa (incluyendo la investigación de separación de uso Felipe/Vicente y el badge de estado de repo, agregadas el 2026-08-28) vivía dentro del doc del monitor solo porque el código las sirve desde el mismo `HttpListener` — no porque sean conceptualmente parte de "¿el stack está sano?". Son dos preguntas distintas (¿el servidor responde bien? vs. ¿cuánto tiempo se trabajó en qué?) que terminaron en el mismo documento por un detalle de implementación, no por relación real.

**Corregido:** creado `docs/06-operacion/04-bitacora-horas.md` con todo el contenido de la bitácora movido ahí completo (no resumido); `03-monitor-estado.md` recortado a solo lo que es realmente el monitor de estado, con una nota corta al final aclarando que el mismo servidor sirve `/bitacora` pero que es otra cosa, documentada aparte. Actualizadas las referencias en `docs/06-operacion/README.md`, `README.md` raíz y `scripts/README.md`. Verificado con el script de links/anclas: 0 errores en 74 `.md` de los cuatro repos (uno más que antes, por el archivo nuevo).

**Lección general, no solo de este archivo:** compartir un proceso/servidor por conveniencia de implementación no es motivo para mezclar la documentación de dos funcionalidades distintas en el mismo documento — el criterio para dónde va algo debe ser "¿de qué trata conceptualmente?", no "¿en qué archivo de código vive?".
