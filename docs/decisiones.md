...
- **Aclaración explícita:** usar Tailscale para llegar directo al puerto de Ollama no contradice la decisión ya cerrada de "no exponer el puerto crudo de Ollama" (`acceso-remoto.md`) — esa decisión era sobre exposición pública vía Cloudflare, y Tailscale es una red privada cerrada, no exposición pública.
...
- Se creó `docs/qwen-code-a-fondo.md` con el detalle completo (tabla de funciones de Qwen Code cruzadas contra los números reales de `modelo-elegido.md`, y los tres escenarios de conexión). Se actualizaron `arquitectura-piloto.md`, `acceso-remoto.md`, `mantenimiento.md` (Tailscale y Qwen Code agregados a la tabla de costos) y `README.md` para reflejarlo.
...
- **Re-chequeado si hay un Qwen3-Coder chico (verificado en `github.com/QwenLM/Qwen3-Coder`):** no existe ninguna variante bajo los 30B en esa línea (solo 480B-A35B, 30B-A3B y Qwen3-Coder-Next, este sobre una base de 80B). **La elección de Qwen 2.5 Coder 7B se mantiene** — no por falta de revisión, sino porque no hay una opción más chica disponible todavía. Se documentó en `modelo-elegido.md` como una confirmación cerrada, no como una duda abierta.
...
- **Corrección relacionada, encontrada al planear esto:** el criterio de `almacenamiento.md` decía que los modelos van al HDD porque "se leen una sola vez al cargar" — esa asunción ya no aplica con el diseño de intercambio de modelos bajo demanda (se lee el disco varias veces por sesión, no una vez). Se corrigió el criterio y el default de `02-configurar-ollama.ps1` (`-LetraNVMe` en vez de `-LetraHDD`) para que los modelos vayan al NVMe.
- Se creó `docs/capa-diseno.md` con el detalle completo (incluyendo el loop generar→revisar→corregir, documentado como procedimiento — formalizarlo como Skill queda pendiente de verificar el formato exacto que usan Qwen Code/Goose).
