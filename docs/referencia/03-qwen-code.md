...
Investigando el comando `/model` a fondo se encontraron dos flags que **formalizan de fábrica** parte de lo que `../arquitectura/02-capa-diseno.md` documentó como procedimiento manual:
...
## Comandos de sesión (ver también `../herramientas/03-como-funcionan-los-agentes.md` para la continuidad ante corte de luz)
...
| `/compress` | Reemplaza el historial por un resumen para ahorrar tokens de contexto — relevante dado que el modelo local tiene contexto limitado (ver `../referencia/02-qwen-2.5-coder-7b.md`) |
...
**Recomendación para este piloto:** `default` para uso normal (dado que es un modelo de 7B, con más probabilidad de error que uno grande — ver `../modelo/02-modelo-elegido.md`), `plan` quinientos si se le pide algo grande/riesgoso, `yolo` no se recomienda dado que corre en un equipo compartido.
