...
- [ ] **Modelo (Qwen 2.5 Coder):** revisar en `../ia-local/docs/modelos.md` o directamente en Ollama/Hugging Face si salió una versión nueva de la familia Qwen Coder — la familia se actualiza seguido (ver historia en `fundamentacion-modelo.md`), vale la pena revisar aunque no se migre de inmediato.
...
A pedido explícito de Felipe, se revisó cada pieza del stack (ver `arquitectura-piloto.md` para el diagrama completo):
