...
Sigue el stack ya definido en `../01-arquitectura/01-arquitectura-piloto.md` (esquema completo) y en `../ia-local/docs/04-arquitectura.md` (conceptos generales), adaptado a que acá sí hay una GPU dedicada real.
...
- [ ] Confirmar espacio en disco disponible en **ambos discos** (NVMe + HDD, ver `../01-arquitectura/04-almacenamiento.md`) — modelos + embeddings + capa de diseño + índices de prueba: estimar ~25GB, todo en el NVMe.
...
Uso de una sola persona — Ollama es la opción correcta según `../03-herramientas/04-motor-alternativas.md` (vLLM solo se justifica con concurrencia real de varios usuarios, no es el caso acá).

- [ ] Instalar Ollama (`scripts/pasos/01-instalar-ollama.ps1`).
- [ ] **Configurar `OLLAMA_CONTEXT_LENGTH` explícitamente (crítico, no saltarse).** Con menos de 24GB de VRAM (el caso de esta RTX 5070 12GB), Ollama usa por defecto solo **4K de contexto** — muy por debajo de los **32K reales de entrenamiento** de este modelo (`max_position_embeddings=32768`, confirmado 2026-08-27 en `../02-modelo/02-modelo-elegido.md` — los 100K/131K que se citaban originalmente como "contexto largo" requieren YaRN, que Ollama todavía no expone, y extenderlo sin YaRN degrada calidad en vez de solo arriesgar un error).
- [ ] Configurar `OLLAMA_MODELS` apuntando al NVMe (ver `../01-arquitectura/04-almacenamiento.md` — corregido 2026-08-27, los modelos se intercambian en VRAM varias veces por sesión con la capa de diseño, conviene el disco rápido).
- [ ] Descargar Qwen 2.5 Coder 7B en cuantización Q4_K_M (ver `../02-modelo/02-modelo-elegido.md`) — probar también Q8_0, marcado "best for your GPU" en la verificación de hardware.
...
- [ ] Con esto, Goose ya puede comitear/pushear a GitHub igual que se hace en esta conversación — ver `../03-herramientas/01-herramientas-trabajo.md` § "Conectores a GitHub".

## Paso 1.5 — Herramientas de trabajo para programar

Ver `../03-herramientas/01-herramientas-trabajo.md` para el detalle completo y las alternativas evaluadas.
...
Ver `../01-arquitectura/03-docker-y-recursos.md` antes de este paso — explica por qué **ninguna de estas piezas necesita Docker** (ambas tienen forma nativa oficial en Windows), y el presupuesto real de RAM en un equipo de 16GB.
...
- [ ] Crear la primera cuenta en Open WebUI (queda como admin, cierra el registro público automáticamente — ver `../06-operacion/02-acceso-remoto.md`).
...
Ver `../06-operacion/02-acceso-remoto.md` para el detalle completo de la decisión (Cloudflare Tunnel + dominio propio + Cloudflare Access).
...
Ver `../03-herramientas/02-qwen-code-a-fondo.md` para el detalle completo — solo hace falta si se va a usar Qwen Code/Goose desde un dispositivo fuera de la casa, no para el uso local ni para el acceso por navegador del Paso 3.
...
Ver `../01-arquitectura/02-capa-diseno.md` para el razonamiento completo — cierra la brecha de que el modelo de código es solo texto y no puede evaluar visualmente su propio resultado.
...
Ver `../06-operacion/01-mantenimiento.md` para el detalle completo.
...
Ver `../06-operacion/03-monitor-estado.md` para el detalle completo — un dashboard/JSON de estado, pensado para revisar de un vistazo que todo sigue andando bien, tanto en el mismo equipo como remoto.
...
- [ ] Correr `scripts/pasos/11-prueba-estres.ps1` (`.bat`) — mide tok/s real, estabilidad bajo carga sostenida, y el límite real de contexto (cierra el pendiente de `../02-modelo/02-modelo-elegido.md` sobre si el límite de 32K de Ollama es real o solo el default). Ver `../05-pruebas/02-pruebas-rendimiento.md` para cómo interpretar los resultados.

## Siguiente documento

Una vez instalado y con la prueba de estrés corrida, pasar a `../05-pruebas/01-plan-pruebas.md` para la evaluación de **calidad** con tareas de programación concretas, y registrar todos los resultados (rendimiento + calidad) en `../05-pruebas/resultados.md`.

## Antes de empezar: entender qué hace cada script

Ver `02-aprendizaje-scripts.md` — explica qué hace cada `.ps1` y por qué, no solo el comando a correr. Es parte del objetivo del proyecto (entender, no solo ejecutar), igual que `../02-modelo/01-fundamentacion-modelo.md` con la elección del modelo.
