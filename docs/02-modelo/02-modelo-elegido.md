...
| **"Contexto seguro" según el sitio** | **100K** (no los 131K teóricos del modelo — 100K es lo que el sitio calcula que cabe con margen en 12GB de VRAM) |
| Workload "Coding" específico | Grado A, "Runs well" (sin offload), 98,0 tok/s, TTFT 1976ms, "contexto seguro" 100K |

Los 5 workloads evaluados (Chat/Coding/Agentic Coding/Reasoning/RAG) dan **"Runs well" en los 5, sin ningún offload** — a diferencia de otras opciones de esta misma GPU (ver comparación abajo), este modelo no fuerza ningún compromiso de VRAM.

> **Corrección importante (2026-08-27, ver hallazgo completo más abajo y en `../07-referencia/02-qwen-2.5-coder-7b.md`):** este "100K" es un cálculo de cuánto cabe en la VRAM (caché KV), no el contexto con el que el modelo fue entrenado. El `config.json` oficial confirma que el modelo fue entrenado con **32K** (`max_position_embeddings: 32768`) — ir más allá de eso requiere activar YaRN explícitamente (no viene por defecto, y Ollama todavía no lo expone del todo), y previsiblemente degrada la calidad de las respuestas en vez de simplemente fallar. Donde este documento sigue mencionando "100K" más abajo, léase como "lo que cabe en VRAM", no como un límite de calidad verificado.
...
Qwen 3.5 9B saca mejor grado general, pero eso es "grado" de ajuste/calidad global, no específico de código — en el workload "Coding" puntual, Qwen 3.5 9B da 77,1 tok/s con TTFT 2511ms, más lento que Qwen 2.5 Coder 7B (98,0 tok/s), y con menos margen de VRAM disponible para contexto (32K de "ajuste de VRAM" del sitio, contra 100K en Qwen 2.5 Coder 7B — misma advertencia de la nota de arriba aplica a ambos números: son capacidad de memoria, no contexto de entrenamiento verificado de cada modelo). **Se mantiene la elección de Qwen 2.5 Coder 7B** — sigue siendo la mejor opción para el caso de uso específico (programación, buena velocidad, sin forzar offload), ahora confirmado con datos de la GPU exacta, no solo por analogía.

## Qué esperar en la práctica, por tipo de operación (en este equipo exacto)

Los números de arriba (98 tok/s, 32K de contexto real de entrenamiento) no dicen por sí solos qué se puede hacer con esto. Acá se traduce a expectativas concretas, separado por tipo de tarea — para no llegar a instalar sin saber qué pedirle ni qué no pedirle.
...
### Qué tan lejos llega el contexto en la práctica

Dentro de los **32K de contexto con los que el modelo fue realmente entrenado** (ver hallazgo más abajo), eso equivale, muy aproximado (varía mucho según lenguaje y densidad del código), a **unas 500-800 líneas de código típico** — un archivo grande completo, o un par de archivos relacionados de un mismo módulo. La VRAM de esta GPU permitiría cargar hasta 100K (~1.500-2.500 líneas) sin quedarse sin memoria, pero forzar el contexto por sobre los 32K nativos sin YaRN es esperable que degrade la calidad de lo que el modelo "recuerda" del principio, no solo un límite de memoria. No alcanza para un repositorio completo de tamaño mediano/grande de una sola vez — para eso hace falta RAG (recuperar solo las partes relevantes) en vez de meter todo el proyecto en el contexto.
...
| **Depuración con contexto largo** | Dentro de los 32K nativos, puede seguir el hilo de varias funciones/archivos relacionados sin perder de vista el inicio de la conversación | Asumir que rinde igual de bien acercándose a los 100K que la VRAM permite cargar — eso no está verificado, solo el ajuste de memoria; cargar un proyecto entero de una vez y esperar que "entienda todo" tampoco — mejor acotar a los archivos relevantes |
...
| **Español** | El modelo declara amplio soporte de idiomas (familia Qwen, 100+ idiomas a nivel Qwen3.6), pero el benchmark de código citado es en inglés | No hay benchmark propio verificado de calidad en español para esta variante — queda como ítem a probar en `../05-pruebas/01-plan-pruebas.md`, no asumir que rinde igual que en inglés |
...
- [x] Velocidad estimada específica de esta GPU exacta (no proxy) — resuelto 2026-08-26: 98,0 tok/s en Q4_K_M según willitrunai.com. **Sigue pendiente la medición real de primera mano** una vez instalado (ver `../05-pruebas/resultados.md`) — el sitio mismo aclara que son estimaciones, no mediciones.
...
- [x] Contexto usable en la práctica — resuelto con matiz: el sitio confirma que 100K cabe en la VRAM de esta GPU exacta (no los 131K teóricos), pero eso es ajuste de VRAM, no el límite real de calidad — ver el hallazgo completo del ítem siguiente (32K es el contexto real de entrenamiento).
