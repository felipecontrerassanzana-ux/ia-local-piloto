...
4. **Pendiente de confirmar:** la documentación de Cloudflare no deja explícito si los Service Tokens están incluidos sin costo en el plan gratis (a diferencia de Access mismo, que sí está confirmado "$0 forever" hasta 50 usuarios) — antes de construir este camino habría que verificarlo en vivo (crear un token de prueba en el dashboard gratis y confirmar que no pide upgrade), no asumirlo.
...
- [ ] Si más adelante se decide construir C2: confirmar en el dashboard gratis de Cloudflare si los Service Tokens piden upgrade, antes de darlo por viable.
