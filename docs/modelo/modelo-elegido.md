...
- [ ] **Nuevo hallazgo a verificar (2026-08-26):** la página oficial de Ollama (`ollama.com/library/qwen2.5-coder/tags`) lista el contexto de **todas** las variantes de este modelo (incluyendo `7b` y `7b-instruct-q8_0`) como **32K**, no los 131K del modelo original (Hugging Face) ni los 100K estimados por willitrunai.com. Es probable que sea solo el valor por defecto empaquetado en el Modelfile de Ollama (los pesos siguen soportando más, se debería poder subir con `OLLAMA_CONTEXT_LENGTH`/`num_ctx`), pero **hay que confirmarlo empíricamente una vez instalado** — no asumir que se puede llegar a 100K reales solo porque la teoría lo permite. Ver `scripts/verificar-instalacion.ps1`, que incluye un chequeo de `ollama ps` para esto. Tags exactos confirmados para instalar: `qwen2.5-coder:7b` (4,7GB) y `qwen2.5-coder:7b-instruct-q8_0` (8,1GB).
