...
- **Todos los servicios deben arrancar solos al iniciar Windows, sin necesitar que alguien inicie sesión:**
  ...
  - **Monitor de estado** (agregado 2026-08-27, ver `monitor-estado.md`): Tarea Programada `Monitor-Estado-Local`, mismo patrón que Qdrant/Open WebUI — arranca con Windows, corre como `SYSTEM`.
...
**Qué respaldar (lo que no está en git, ver `../arquitectura/arquitectura-piloto.md` punto 4):**
...
- [ ] **Modelo (Qwen 2.5 Coder):** revisar en `../ia-local/docs/modelos.md` o directamente en Ollama/Hugging Face si salió una versión nueva de la familia Qwen Coder — la familia se actualiza seguido (ver historia en `../modelo/fundamentacion-modelo.md`), vale la pena revisar aunque no se migre de inmediato.
...
- [ ] **Revisar el monitor de estado** (`http://localhost:8090/`, ver `monitor-estado.md`) — de hecho, este chequeo mensual es exactamente para lo que existe: un vistazo rápido en vez de revisar cada pieza a mano.

## 4. Costos — confirmado que toda la estructura es gratuita (2026-08-26)

A pedido explícito de Felipe, se revisó cada pieza del stack (ver `../arquitectura/arquitectura-piloto.md` para el diagrama completo):
...
| **Tailscale** | **Gratis** (plan Personal) | Verificado 2026-08-27 en la página oficial de precios: hasta 6 usuarios, dispositivos ilimitados — usado para conectar Qwen Code/Goose remotos, ver `../herramientas/qwen-code-a-fondo.md` |
| Monitor de estado | Gratis | PowerShell puro (`System.Net.HttpListener`, parte de .NET/Windows) — sin librería ni servicio de terceros, ver `monitor-estado.md` |
