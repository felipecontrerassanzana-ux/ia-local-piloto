...
## Segunda ruta: `/bitacora` — cuánto tiempo real se trabajó, no solo si el stack está sano (agregado 2026-08-28)

Distinto de todo lo de arriba: `/estado` y `/` responden "¿está funcionando el servidor ahora?" — `/bitacora` responde "¿cuánto tiempo activo se le dedicó a cada proyecto, qué días, a qué hora?", a partir del historial real de sesiones de Claude Code de la cuenta de Windows que lo genera (no de esta app ni de Qwen Code/Goose — ver el porqué abajo).

**Origen:** Felipe pidió medir cuánto tiempo propio invierte en cada proyecto — se prototipó primero como Artifact de Claude (iteración rápida, varias vueltas de diseño) y, una vez validado el concepto, se portó a un script real de este repo para que no dependa de Claude ni de claude.ai para actualizarse — mismo argumento que ya cerró la sección de arriba sobre `/estado`, aplicado ahora a este caso nuevo.
...
### Estado de repo (local vs. GitHub) por proyecto, agregado 2026-08-28
