...
## Este mismo servidor también sirve otra ruta que no es parte del monitor

`monitor-estado-servidor.ps1` expone además `GET /bitacora` — pero esa ruta no chequea salud del stack, es una funcionalidad completamente distinta (cuánto tiempo se trabajó en cada proyecto) que solo comparte el proceso HTTP por conveniencia de implementación. Documentada aparte en `04-bitacora-horas.md`, para no mezclar dos conceptos que no tienen relación solo porque el código los sirve desde el mismo lugar.
