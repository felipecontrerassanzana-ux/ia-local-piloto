...
- El costo real de este diseño es **latencia de intercambio** (unos segundos al cambiar de tarea), no falta de espacio — ver `04-almacenamiento.md` para por qué eso además cambió dónde viven los modelos en disco (NVMe, no HDD).
...
- [x] Formalizar el loop como comando invocable — resuelto para Qwen Code (comando personalizado `.qwen/commands/diseno/revisar.md`, ver `../referencia/03-qwen-code.md`); pendiente el equivalente en Goose (Recipe, formato exacto sin verificar aún, ver `../referencia/04-goose.md`).
...
- [ ] Medir en la práctica si el contexto real disponible (ver `../referencia/02-qwen-2.5-coder-7b.md` — 32K confirmado, más allá de eso degrada calidad sin YaRN) alcanza para incluir capturas + reglas de diseño + código sin degradar la respuesta.
